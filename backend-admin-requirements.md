# Muslim Community Lithuania Backend + Admin Panel Requirements

Build a MERN stack backend and admin panel for the Muslim Community Lithuania mobile app.
...
- File uploads: local storage for development, cloud-ready abstraction for production
...
For announcements and quiz questions, return only records matching `lang`.
...
- Admin panel should also support bulk import for a date range, month, or CSV/JSON timetable.
...
Admin-side quiz records should also store:

```ts
interface QuizQuestionDocument extends QuizQuestion {
  lang: 'en' | 'ru';
  isActive: boolean;
  createdAt: string;
  updatedAt: string;
...
- send push notifications only to active push registrations where `lang` matches the announcement language
- notification title: announcement title
- notification body: announcement excerpt
...
- table with image, title, lang, status, date, event date
- create/edit form
...
- table with question, lang, category, active status
- create/edit form
- language select
- category select:
  - `aqeedah`
...
PORT=4000
MONGODB_URI=mongodb://localhost:27017/jumuah
...
- Store rich text as sanitized HTML in `descriptionHtml`.
...
- Published announcements can send push notifications by language
