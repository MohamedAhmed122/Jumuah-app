...
# Community multi-mosque feeds and mosque cities

Mosque records now require a stable `city` string. Public mosque lists include it and may be filtered with an exact, case-insensitive city query:

```http
GET /api/locations/mosques?city=Vilnius
```

Announcement and event feeds accept either the legacy singular query or a comma-separated multi-mosque query:

```http
GET /api/community/announcements?mosqueIds=id1,id2,id3&lang=lt
GET /api/community/events?mosqueIds=id1,id2,id3&lang=lt&deviceId=device-123
GET /api/community/announcements?mosqueId=id1&lang=lt
GET /api/community/events?mosqueId=id1&lang=lt&deviceId=device-123
```

`mosqueIds` contains 1-20 valid 24-character MongoDB ObjectIds. Whitespace around comma-separated IDs is ignored and repeated IDs are collapsed. Sending both `mosqueId` and `mosqueIds`, sending an empty component, omitting both parameters, using an invalid ID, or exceeding 20 supplied IDs returns HTTP 400 with the standard `VALIDATION_ERROR` body. Feed items match any selected mosque and each MongoDB document is returned once, including when it belongs to several selected mosques. Existing ordering, published/expiry filtering, response arrays, Lithuanian localization with English fallback, and event attendance fields are unchanged.

Event detail remains singular and verifies that the event belongs to the supplied `mosqueId`. When `mosqueId` is supplied to `POST /api/community/events/:id/join`, it must be one of the event's associated mosques. Join remains idempotent and returns `joined: true` with the updated `attendeeCount`.

Existing mosque records must be assigned a city before deployment. `npm run seed:locations` backfills the starter Vilnius mosque; the demo seed assigns cities to all demo mosques.
